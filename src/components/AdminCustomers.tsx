...
export const AdminCustomers = () => {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`/api/customers/manage.php?id=${id}`, { method: 'DELETE' });
            fetchCustomers();
        } catch (err) {
            console.error(err);
    ...
    const handleSuspend = async (id: number, currentStatus: boolean) => {
        try {
            try {
                await fetch(`/api/customers/suspend.php?id=${id}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
        ...
        const handleUpdate = async (e: React.FormEvent) => {
            ...
            setIsSubmitting(true);
            try {
                await fetch(`/api/customers/manage.php?id=${editingCustomer.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
        ...
        const verifySite = async (id: number) => {
            setVerifyingId(id);
            try {
                const res = await fetch(`/api/customers/verify.php?id=${id}`, { method: 'POST' });
                const data = await res.json();
                            ...
                            >
                                <div className="flex-1 space-y-1 min-w-0">
                                    <div className="flex items-center gap-3 flex-wrap">
                                        <h3 className="text-xl font-bold text-white truncate max-w-[200px] md:max-w-none">{c.company_name}</h3>
                                        {c.is_suspended && (
                                            <span className="px-2 py-0.5 bg-red-500/20 text-red-400 text-xs rounded-full border border-red-500/20 flex items-center gap-1 shrink-0">
                                                <Ban size={10} /> Askıda
                                            </span>
                                        )}
                                        <div className={clsx(
                                            "px-2 py-0.5 rounded-full text-xs font-medium border flex items-center gap-1.5 shrink-0",
                                            isOnline
                                    ...
                                    <div className="text-sm text-gray-400 flex flex-wrap gap-x-4 gap-y-1">
                                        <span className="truncate max-w-[150px]">{c.contact_name}</span>
                                        <span className="text-gray-600 hidden sm:inline">•</span>
                                        <span className="truncate max-w-[200px]">{c.email}</span>
                                        <span className="text-gray-600 hidden sm:inline">•</span>
                                        <a href={`https://${c.domain}`} target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-300 transition-colors truncate max-w-[150px]">
                                            {c.domain}
                                        </a>
                                    </div>
                                    <div className="text-xs text-gray-500 font-mono mt-2 flex items-center gap-2">
                                        ID: <span className="bg-white/5 px-1.5 py-0.5 rounded text-gray-300 truncate max-w-[150px]">{c.site_id}</span>
                                    </div>
                                </div>
